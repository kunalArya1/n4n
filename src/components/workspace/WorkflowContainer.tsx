...
export default function WorkflowContainer({
  initialWorkflows, workspaceId, workspaceName,
  ...
  const filteredWorkflows = useMemo(() => {
    ...
    result.sort((a, b) => {
      switch (sortBy) {
        ...
        case "nodes": return b._count.nodes - a._count.nodes;
        default: return 0;
      }
    });
    return result;
  ...
  function handleSoftDelete(id: string) {
    ...
    setTrashWorkflows((prev) => [
      { ...wf, isDeleted: true, deletedAt: new Date().toISOString() }, ...prev,
    ]);
  ...
  function handleBatchSoftDelete() {
    ...
    setTrashWorkflows((prev) => [
      ...toTrash.map((wf) => ({ ...wf, isDeleted: true, deletedAt: new Date().toISOString() })),
      ...prev,
    ]);
  ...
  function handleStatusChanged(id: string, status: WorkflowStatus) {
    setWorkflows((prev) =>
      prev.map((wf) => wf.id === id ? { ...wf, status, updatedAt: new Date().toISOString() } : wf),
    );
  }
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-4">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-muted-foreground mt-1 text-sm">
              {showTrash ? "Access deleted workflows" : (
          ...
          <div className="order-2 flex max-w-full min-w-45 flex-1 items-center gap-2 sm:order-0">
            <WorkflowSearchBar query={searchQuery} onQueryChange={setSearchQuery}
          ...
          <div className="flex shrink-0 flex-wrap items-center gap-2">
            ...
            <Button variant={showTrash ? "secondary" : "outline"} size="sm" className="gap-2 text-xs"
              onClick={() => {
      ...
      {!showTrash ? (
        <WorkflowGrid workflows={filteredWorkflows} createOpen={createOpen}
          onCreateOpenChange={setCreateOpen}
          onCreated={(wf) => setWorkflows((prev) => [wf, ...prev])}
          onRenamed={(wf) => setWorkflows((prev) => prev.map((w) => (w.id === wf.id ? { ...w, name: wf.name } : w)))}
          onDeleted={handleSoftDelete} onStatusChanged={handleStatusChanged}
          workspaceId={workspaceId} selectedIds={selectedIds} onSelectionChange={setSelectedIds} />
      ) : (
        <WorkflowTrashGrid workflows={filteredTrash} onRestored={handleRestore}
          onPermanentDeleted={handlePermanentDelete}
          selectedIds={trashSelectedIds} onSelectionChange={setTrashSelectedIds} />
      )}
